...
<div class="card shadow">
  ...
  <div class="card-body">
    ...
    <p>
      ...
      {{ $appointment->doctor->name }} (especialidad {{ $appointment->specialty->name }}) 
      para el dia {{ $appointment->scheduled_date }}:
    </p>
    @elseif ($role == 'doctor')
    <p>
      Estás a punto de cancelar tu cita con el paciente 
      {{ $appointment->patient->name }} (especialidad {{ $appointment->specialty->name }}) 
      para el dia {{ $appointment->scheduled_date }} 
    ...
    <form action="{{ url('/appointments/'.$appointment->id.'/cancel') }}" method="POST">
      @csrf
    <div>
      <label for="justification">Por favor cuéntamos el motivo de la cancelación:</label>
